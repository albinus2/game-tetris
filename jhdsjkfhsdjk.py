...
fps = 25
window_w, window_h = 410, 1000
block, cup_h, cup_w = 40, 20, 10
...
def pauseScreen():
    menu = pygame_menu.Menu('Menu', 410, 1000,
                        theme=pygame_menu.themes.THEME_ORANGE)
    menu.add.button('Play', runTetris)
...
def main():
    ...
    basic_font = pg.font.SysFont('Comic Sans MS', 20)
    big_font = pg.font.SysFont('verdana', 45)
    pg.display.set_caption('Падающие Блоки')
    showText('Падающие Блоки')
...
def showText(text):
    ...
    pressKeySurf, pressKeyRect = txtObjects('Нажмите любую клавишу для начала', basic_font, title_color)
...
def drawInfo(points):
    ...
    pointsSurf = basic_font.render(f'бранно полос: {points}', True, txt_color)
    pointsRect = pointsSurf.get_rect()
    pointsRect.topleft = (50, 50)
    display_surf.blit(pointsSurf, pointsRect)

    pausebSurf = basic_font.render('Пробел: Меню', True, txt_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (250, 50)
    display_surf.blit(pausebSurf, pausebRect)
    EscbSurf = basic_font.render('Esc: Выход', True, txt_color)
    EscbRect = EscbSurf.get_rect()
    EscbRect.topleft = (150, 100)
